...
def run(weights=ROOT / 'best.onnx',  # model.pt path(s)
    ...
    source, key = get_img('jiyoon2.jpeg')
    save_img = not nosave 
    ...
    # Dataloader
    path, im, im0s , s = LoadImages(source, key, img_size=imgsz, stride=stride, auto=pt)
    bs = 1  # batch_size
...
#       gn = torch.tensor(im0.shape)[[1, 0, 1, 0]]  # normalization gain whwh
        ...
        image = cv2.resize(im0, (1000, 1000))
        cv2.imshow(f'{names[c]} {conf:.2f}', image)
        cv2.waitKey(6000)
